...
const WholesalerMarketModal = ({ modalVisible, selectedItem, setModalVisible, formatDate, formatTime, onClose }) => {
    ...
    const getHistoryByTimeframe = async (wholesalerId, categoryId, subCategoryName,timeframe) => {
        try {
            const response = await axios.get(`${Base_url}b2bUser/wholesalers/${wholesalerId}/category/${categoryId}/subcategory/${subCategoryName}/history/${timeframe}`);
    ...
    useEffect(() => {
        if (selectedItem && selectedItem.category && selectedItem.category.length > 0) {
          const wholesalerId = selectedItem.id;
          const categoryId = selectedItem.category[0]?._id; // Assuming you want the first category
          const subCategoryName = selectedItem.category[0]?.sub_category && selectedItem.category[0]?.sub_category.length > 0
            ? selectedItem.category[0]?.sub_category[0]?.name
            : null; // Assuming you want the first subcategory
      
          fetchUserMandis(wholesalerId, categoryId, subCategoryName);
        }
      }, [selectedItem]);
...
export default WholesalerMarketModal;
